interface SystemPromptParams {
  timeString: string;
  userName: string;
  robotName: string;  // 添加机器人名字参数
}

export const getSystemPrompt = ({ timeString, userName, robotName }: SystemPromptParams): string => {
  ...
  8. 你的名字只有 "${robotName}"，没有其他名字或别名。如果用户用其他名字（如 EMO、Jarvis 等）来称呼你或者向你提问时，请礼貌幽默地纠正他们，告诉他们你叫 "${robotName}"。
